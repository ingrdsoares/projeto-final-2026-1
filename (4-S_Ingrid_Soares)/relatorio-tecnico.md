...
- **Deployment:** A stack de detecção (Wazuh) é empacotada via Docker Compose para garantir reprodutibilidade. O Agente e a API rodam em Python, permitindo fácil integração com LLMs via LangChain.
- **Confiabilidade:** Implementou-se a análise de **Falsos Positivos (Noise Analysis)**, simulando ações administrativas legítimas para garantir que o sistema não gere alertas excessivos.
