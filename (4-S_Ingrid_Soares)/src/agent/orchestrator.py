...
class PurpleOrchestrator:
    ...
    def run_validation_cycle(self, auto_fix: bool = False):
        ...
        for ttp in TTP_LIST:
            ...
            # 2. Check Detection
            self.log_reasoning(
                f"Action: Querying Wazuh API for alerts related to {ttp}."
            )
            det_res = self.tools.check_detection(ttp)

            if det_res["detected"]:
                self.log_reasoning(
                    f"Conclusion: TTP {ttp} was detected. Security control is effective."
                )
                self.results[ttp] = "DETECTED"
            else:
                self.log_reasoning(
                    f"Conclusion: TTP {ttp} was NOT detected. Blind spot identified."
                )
                ...
                if auto_fix:
                    ...
                    # 3. Propose and Apply
                    rule = self.tools.propose_detection_rule(
                        ttp, "Simulated Sysmon logs"
                    )
                    ...
                    if re_det["detected"]:
                        self.log_reasoning(
                            f"Conclusion: TTP {ttp} is now DETECTED. Remediation successful."
                        )
                        self.results[ttp] = "DETECTED"
                    else:
                        self.log_reasoning(
                            f"Conclusion: TTP {ttp} still NOT detected. Manual intervention required."
                        )
                        self.results[ttp] = "MISSED"
    ...
    def validate_noise(self):
        ...
        for action in actions:
            ...
            if is_alerted:
                self.log_reasoning(
                    f"Result: [ NOISE ] Action {action} triggered a false positive alert."
                )
                noise_results[action] = "FALSE POSITIVE"
            else:
                self.log_reasoning(
                    f"Result: [ CLEAN ] Action {action} was correctly ignored."
                )
